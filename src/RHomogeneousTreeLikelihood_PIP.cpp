...
void RHomogeneousTreeLikelihood_PIP::sortData(std::vector<int> &array,std::vector<int> &indeces){
...
void RHomogeneousTreeLikelihood_PIP::computeSubtreeLikelihood(const std::vector<int> &nodeList) {
    ...
    //===============================================================
    // m@x
    std::vector<int> dist_to_root(nodeList.size());
    std::vector<int> indeces(nodeList.size());
    for(int i=0;i<nodeList.size();i++){
        indeces.at(i)=i;
        int id = nodeList.at(i);
        dist_to_root.at(i)=0;
        tshlib::VirtualNode *node =  this->utree_->getNode(id);
        if(node != nullptr){
    ...
    std::vector<int> full_node_list(nodeList.size());
    for(int i=0;i<nodeList.size();i++){
        full_node_list.at(i)=nodeList.at(indeces.at(i));
    }
    //===============================================================
    ...
    for (auto &nodeID:full_node_list) {
    //==============================================
...
void RHomogeneousTreeLikelihood_PIP::computeSubtreeLikelihood(std::map<int, VVVdouble> *likelihoods,
    ...
    for (auto &nodeID:nodeList) {
        ...
        //if (!utree_->getNode(treemap_.left.at(nodeID))->isTerminalNode()) {
        if (!_utree__topology.getNode(treemap_.left.at(nodeID))->isTerminalNode()) {
...
void RHomogeneousTreeLikelihood_PIP::getNodeListPostOrder_(std::vector<int> &nodeList, int startNodeID) const {
    ...
    if (!node->isLeaf()) {
        ...
        std::cout<<"postorder: "<<node->getName()<<","<<node->getId();
        std::cout<<": ("<<node->getSons().at(0)->getName()<<",";
        std::cout<<node->getSons().at(1)->getName()<<")"<<std::endl;

        nodeList.push_back(startNodeID);
    } else {

        std::cout<<"postorder: "<<node->getName()<<","<<node->getId()<<"["<<node->getFather()->getName()<<"]"<<std::endl;
...
double RHomogeneousTreeLikelihood_PIP::computeLikelihoodWholeAlignmentEmptyColumn(std::map<int, VVVdouble> *ts_lkemptydata,
    ...
    for (int idxNode = 0; idxNode < likelihoodNodes.size(); idxNode++) {
        ...
        std::vector<VVVdouble *> lk_sons_empty(2);
        //if (!utree_->getNode(treemap_.left.at(likelihoodNodes[idxNode]))->isTerminalNode()) {
        if (!_utree__topology.getNode(treemap_.left.at(likelihoodNodes[idxNode]))->isTerminalNode()) {
...
double RHomogeneousTreeLikelihood_PIP::computeN1DerivativeLikelihood(const std::string &variable) {
    ...
    double lk_2 = 0;
    double result = 0;
    double h = 0.01*branch->getDistanceToFather();
...
double RHomogeneousTreeLikelihood_PIP::computeN1DerivativeLikelihood(const std::string &variable) {
    ...
    double lk_4 = 0;
    double result = 0;
    double bl = branch->getDistanceToFather();
    double h = 0.01*bl;

    //double BranchLength_1 = branch->getDistanceToFather();
    double BranchLength_1 = (bl - 2*h);
    double BranchLength_2 = (bl - h);
    double BranchLength_3 = (bl + h);
    double BranchLength_4 = (bl + 2*h);

    if(BranchLength_1<0.0 || BranchLength_2 <0.0 || BranchLength_3<0.0 || BranchLength_4<0.0){
        LOG(FATAL) << "ERROR: computeN1DerivativeLikelihood";
        exit(EXIT_FAILURE);
    }
    ...
    this->setParameterValue(variable, BranchLength_3);

    fireParameterChanged(getParameters());

    lk_3 = -getValue();

    this->setParameterValue(variable, BranchLength_4);

    fireParameterChanged(getParameters());

    lk_4 = -getValue();

    double op = -exp(lk_4-lk_0) + 8*exp(lk_3-lk_0) - 8*exp(lk_2-lk_0) + exp(lk_1-lk_0);

    result = op / (12*h); // scaled derivative to avoid underflow/overflow
